...
describe('Level 4', function() {
  ...
  var parsed = ts.parse()
  let nestedData = parsed.getNestedArrayObject('classifier');

  it('quantidade de linhas do resultado', function() {
    expect(parsed.result.length).to.equals(59);
  });
  
  it('verificacao do primeiro item do resultado do parser', function() {
    expect(parsed.result[0]).to.eql({description: "A T I V O", classifier: "1000000000", openingBalance: 5869359.63, debit: 13988798.89, credit: 14478791.43,  finalBalance: 5379367.09 });
  });
  
  it('identificacao do parent "classifier"', function() {
    let children = parsed.result[parsed.result.length - 1].classifier;
    expect(parsed.findParentId('classifier', children)).to.equals('2102000000');
    expect(parsed.findParentId('classifier', '1103010200')).to.equals('1103000000');
  });
});